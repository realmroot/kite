...
if [ -n "${KITE_E2E_DB_PATH:-}" ]; then
  ...
  DB_PATH="$(mktemp "${TMPDIR:-/tmp}/lightkite-e2e.XXXXXX")"
fi
...
export KITE_ENCRYPT_KEY="${KITE_E2E_ENCRYPT_KEY:-lightkite-e2e-encryption-key}"
export OIDC_ISSUER="${KITE_E2E_OIDC_ISSUER:-https://localhost:5556}"
export OIDC_CA_FILE="${KITE_E2E_OIDC_CA_FILE:-${KITE_E2E_OIDC_CERT_DIR:-${TMPDIR:-/tmp}/lightkite-e2e-oidc}/ca.crt}"
export OIDC_CLIENT_ID="${KITE_E2E_OIDC_CLIENT_ID:-lightkite-e2e}"
export OIDC_CLIENT_SECRET="${KITE_E2E_OIDC_CLIENT_SECRET:-lightkite-e2e-secret}"
export OIDC_PROVIDER_NAME="${KITE_E2E_OIDC_PROVIDER_NAME:-Lightkite E2E Identity}"
...
if [ -n "${KITE_E2E_CONFIG_FILE:-}" ]; then
  ...
  E2E_CONFIG_PATH="$(mktemp "${TMPDIR:-/tmp}/lightkite-e2e-config.XXXXXX")"
  printf '{}\n' >"${E2E_CONFIG_PATH}"
  export KITE_CONFIG_FILE="${E2E_CONFIG_PATH}"
fi
export PORT
KITE_E2E_HELM_DIR="${KITE_E2E_HELM_DIR:-$(mktemp -d "${TMPDIR:-/tmp}/lightkite-e2e-helm.XXXXXX")}"
