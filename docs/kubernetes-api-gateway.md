...
Kite exposes the selected cluster's standard Kubernetes HTTP API below a
...
after `/kubernetes` is the canonical upstream Kubernetes resource path; Kite
...
Kite maintains one credential-free connection pool per cluster. A lightweight
per-request wrapper supplies the user's token, so the number of TLS connection
pools and discovery transports scales with clusters rather than users. Kite
