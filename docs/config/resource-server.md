...
Lightkite is a dashboard client, not an Agent Resource Server. Agent access is
provided by a Cluster Inventory access provider such as Kube Cluster Hub.

Lightkite discovers access providers from standard `ClusterProfile` resources. It
...
Lightkite contains no Resource Server signing key, DPoP replay store, Agent token
