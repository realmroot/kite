...
Kite does not provide an application-RBAC management API. Kubernetes resource
...
the API server's configured username and groups claims. Kite forwards the
current user's signed ID token and does not impersonate a different principal.

Kite-owned shared metadata has a separate, deliberately narrow policy:
