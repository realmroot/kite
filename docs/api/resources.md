...
Lightkite's browser client uses the Kubernetes API itself. Lightkite does not publish or
...
uses the CRD's plural and scope. Adding a CRD therefore does not add a Lightkite
handler.
...
## Lightkite-specific resource operations

Lightkite retains narrow APIs only when the capability is not one Kubernetes
resource operation:
...
resource). Before reading Lightkite's history database, the backend submits a
...
and success or failure metadata, but Lightkite does not persist Secret YAML bodies or
